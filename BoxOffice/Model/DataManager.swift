...
enum DataManager {
    static func boxOfficeTransferDailyBoxOfficeData(boxOffice: BoxOffice) -> [DailyBoxOfficeData] {
        return boxOffice.boxOfficeResult.dailyBoxOfficeList.map {
            let decimalAudienceCount = FormatManager.bringDecimalString($0.audienceCount)
            let decimalaudienceAccumulation = FormatManager.bringDecimalString($0.audienceAccumulation)
